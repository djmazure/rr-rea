    ...
    async def run(self):
        while self._alive:
            ...
            if int(self.dut.dpram_we_o.value) == 1:
                addr = int(self.dut.dpram_addr_o.value)
                val = int(self.dut.dpram_din_o.value)
                self.cells[addr] = val
...
async def test_rea_req_100_wr_ptr_free_runs_unarmed(dut):
    ...
    snapshots: list[int] = []
    for _ in range(20):
        snapshots.append(int(dut.wr_ptr_o.value))
        await RisingEdge(dut.sample_clk_i)
    ...
    assert int(dut.armed_o.value) == 0, "armed should be 0 (never armed)"
...
async def test_rea_req_101_arm_does_not_reset_wr_ptr(dut):
    ...
    await ClockCycles(dut.sample_clk_i, 50)
    pre_arm_wr_ptr = int(dut.wr_ptr_o.value)
    ...
    dut.trig_value_i.value = 0
    dut.trig_mask_i.value = 0  # mask=0 → trigger-hit always (auto-trigger)
    await _pulse(dut.arm_pulse_i, dut, 1)

    # Sample wr_ptr the cycle AFTER arm took effect.
    post_arm_wr_ptr = int(dut.wr_ptr_o.value)
...
async def test_rea_req_102_trig_ptr_captures_wr_ptr(dut):
    ...
    dut.probe_i.value = 0x42
    dut.pretrig_len_i.value = 8
    dut.posttrig_len_i.value = 8
    dut.trig_value_i.value = 0x42
    ...
    await ReadOnly()
    matching_sample_ptr = int(dut.wr_ptr_o.value)
    for _ in range(20):
        await RisingEdge(dut.sample_clk_i)
        await ReadOnly()
        if int(dut.triggered_o.value) == 1:
            break
    else:
        assert False, (
...
async def test_rea_req_103_dpram_at_trig_ptr_is_trigger_sample(dut):
    ...
    dut.pretrig_len_i.value = 16
    dut.posttrig_len_i.value = 16
    dut.trig_value_i.value = 0x42
    dut.trig_mask_i.value = 0xFF
    ...
    trig_ptr = int(dut.trig_ptr_o.value)
    assert int(dut.triggered_o.value) == 1, "expected trigger to have fired"
...
async def test_rea_req_104_start_ptr_arithmetic(dut):
    ...
    dut.trig_value_i.value = 0x55
    dut.trig_mask_i.value = 0xFF
    dut.probe_i.value = 0x55  # always-match trigger

    # Warm up (also lets wr_ptr move into a wrap-aware position).
    await ClockCycles(dut.sample_clk_i, 100)
    ...
    for _ in range(deadline):
        ...
        if int(dut.done_o.value) == 1:
            break
    assert int(dut.done_o.value) == 1, "done never asserted"
...
async def test_rea_req_106_no_uninit_zeros_when_prewarmed(dut):
    ...
    for _ in range(DEPTH + 200):
        dut.probe_i.value = (counter & 0xFF) | 0x100
        await RisingEdge(dut.sample_clk_i)
        counter += 1
...
async def test_rea_req_400_external_trigger_in_fires_capture(dut):
    ...
    prev_wr_ptr = int(dut.wr_ptr_o.value)
...
async def test_rea_req_500_decimation_zero_stores_every_cycle(dut):
    ...
    dut.trig_value_i.value = 0xFFF
    dut.trig_mask_i.value  = 0xFFF
    dut.decim_ratio_i.value = 0    # ← no decimation
    dut.trigger_i.value = 0
    ...
    we_samples = []
    for _ in range(16):
        we_samples.append(int(dut.dpram_we_o.value))
        await RisingEdge(dut.sample_clk_i)
...
async def test_rea_req_501_decimation_n_stores_one_in_n_plus_one(dut):
    ...
    high_count = 0
    wr_ptr_first = int(dut.wr_ptr_o.value)
    for _ in range(24):
        if int(dut.dpram_we_o.value) == 1:
            high_count += 1
        await RisingEdge(dut.sample_clk_i)
    wr_ptr_last = int(dut.wr_ptr_o.value)
...
async def _assert_no_fire(dut, n: int, msg: str) -> None:
    ...
    for _ in range(n):
        await RisingEdge(dut.sample_clk_i)
        assert int(dut.triggered_o.value) == 0, msg
...
async def test_rea_req_610_rising_edge_fires_only_on_0_to_1(dut):
    ...
    dut.trig_mask_i.value = 0x001  # watch bit 0
    dut.trig_value_i.value = 0     # value is don't-care for edge ops
    dut.trig_mode_i.value = _mode(TRIG_OP_RISE)

    # Hold bit 0 HIGH from before arm so probe_prev_r latches '1'.
    dut.probe_i.value = 0x001
    await ClockCycles(dut.sample_clk_i, 5)
    await _pulse(dut.arm_pulse_i, dut, 1)
...
async def test_rea_req_611_falling_edge_fires_only_on_1_to_0(dut):
    ...
    dut.trig_mask_i.value = 0x001
    dut.trig_value_i.value = 0
    dut.trig_mode_i.value = _mode(TRIG_OP_FALL)
...
async def test_rea_req_620_less_than_fires_strictly_below(dut):
    ...
    dut.trig_mask_i.value = 0xFFF
    dut.trig_value_i.value = 0x100
    dut.trig_mode_i.value = _mode(TRIG_OP_LT)

    dut.probe_i.value = 0x100  # equal → no fire (strict <)
    await ClockCycles(dut.sample_clk_i, 5)
    await _pulse(dut.arm_pulse_i, dut, 1)
    await _assert_no_fire(dut, 5, "REA-REQ-620: LT fired on equality")

    dut.probe_i.value = 0x150  # greater → no fire
    await _assert_no_fire(dut, 5, "REA-REQ-620: LT fired on greater-than")

    dut.probe_i.value = 0x0FF  # less → fire
...
async def test_rea_req_621_greater_than_fires_strictly_above(dut):
    ...
    dut.trig_mask_i.value = 0xFFF
    dut.trig_value_i.value = 0x100
    dut.trig_mode_i.value = _mode(TRIG_OP_GT)
    ...
    dut.probe_i.value = 0x0FF  # less → no fire
    await _assert_no_fire(dut, 5, "REA-REQ-621: GT fired on less-than")

    dut.probe_i.value = 0x150  # greater → fire
...
async def test_rea_req_630_not_equal_fires_on_any_bit_difference(dut):
    ...
    dut.trig_mask_i.value = 0xFFF
    dut.trig_value_i.value = 0x0AA
    dut.trig_mode_i.value = _mode(TRIG_OP_NE)

    dut.probe_i.value = 0x0AA  # equal → no fire
    await ClockCycles(dut.sample_clk_i, 5)
    await _pulse(dut.arm_pulse_i, dut, 1)
...
async def test_rea_req_600b_host_literal_mode_0x01_is_eq(dut):
    ...
    dut.trig_mask_i.value = 0xFFF
    dut.trig_value_i.value = 0x099
    dut.trig_mode_i.value = 0x01  # host literal: value_match, op=EQ

    dut.probe_i.value = 0x000  # mismatch → no fire
    await ClockCycles(dut.sample_clk_i, 5)
    await _pulse(dut.arm_pulse_i, dut, 1)
    await _assert_no_fire(dut, 5, "mode 0x01 fired on a non-matching probe")

    dut.probe_i.value = 0x099  # exact match → fire
    assert await _fire_within(dut, 6), (
